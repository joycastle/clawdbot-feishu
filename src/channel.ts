...
export const feishuPlugin: ChannelPlugin<ResolvedFeishuAccount> = {
  ...
  agentPrompt: {
    messageToolHints: () => {
      ...
      `- **飞书项目 API (localhost:18793)**: 当用户询问工作项、缺陷、任务相关内容时，用 curl 调用：`,
      `  - \`curl http://127.0.0.1:18793/types\` - 获取工作项类型（需求/缺陷/任务等）`,
      `  - \`curl http://127.0.0.1:18793/workitems?typeKey=issue\` - 查询缺陷列表`,
      `  - \`curl http://127.0.0.1:18793/workitems?typeKey=story\` - 查询需求列表`,
      `  - \`curl http://127.0.0.1:18793/workitem/123?typeKey=issue\` - 获取单个工作项详情`,
      `  - \`curl -X POST http://127.0.0.1:18793/workitem -H "Content-Type: application/json" -d '{"typeKey":"issue","name":"标题"}'\` - 创建工作项`,
      `  - \`curl -X POST http://127.0.0.1:18793/workitem/123/comment -H "Content-Type: application/json" -d '{"content":"评论内容"}'\` - 添加评论`,
